...
const char * const LIMRACOIN_CONF_FILENAME = "limracoin.conf";
const char * const LIMRACOIN_PID_FILENAME = "limracoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "limracoind -foo=bar
...
#else
    const char* pszModule = "limracoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Limracoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Limracoin
    // Mac: ~/Library/Application Support/Limracoin
    // Unix: ~/.limracoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Limracoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Limracoin";
#else
    // Unix
    return pathRet / ".limracoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LIMRACOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", LIMRACOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Limracoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Limracoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Limracoin Core developers";
    }
