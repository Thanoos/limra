...
###   limracoincore.org and limracoin.org and compares them.
...
WORKINGDIR="/tmp/limracoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://limracoincore.org"
HOST2="https://limracoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="limracoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'limracoin-',
...
if [ $? -ne 0 ]; then
   echo "limracoin.org failed to provide signature file, but limracoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "limracoin.org and limracoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Limracoin Core binary release signing key installed?"
   fi
